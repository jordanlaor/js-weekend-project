...
function nextNumber(num1, num2, num3) {
  return num1 + num2 + num3;
}

function tribonacci(signature, n) {
  if (n <= 0 || signature.forEach((element) => element < 0) || signature.length !== 3) {
    return [];
  }
  if (n < 3) {
  ...
  while (signature.length < n) {
    ...
    signature.push(nextNumber(signature[sigLength - 1], signature[sigLength - 2], signature[sigLength - 3]));
  }
  return signature;
}
